...
## Cross Validation
To evaluate our model further we will use k-fold cross validation. We have 190 countries with data so we will split our data into 19 folds to have 10 observations in each fold. 
...
enframe(r_sq_results,
  ...
  theme(axis.title.y = element_blank())
```
How predictive is your model? How much does the performance vary between folds? Is there any evidence of overfitting?


According to the graph, the performance is fairly consistent between each fold, with the exception of fold 3 that has an $R^2$ value of approximately 1.6. Based on the average $R^2$ value and second graph displaying the countries average life expectancy vs average healthcare spending, the linear model doesn't appear to be particularly underfit or overfit. However, there are couple of data points that are below the trend line, specifically from Africa, which indicates that our model is better at predicting countries with a higher life expectancy. 
