...
>>>>>>> febb80b544ca2a58166cbfba9470712dc2e9f909
```{r}
data_transform <- 
  joined_data_long |>
  group_by(country) |>
...
To evaluate our model further we will use k-fold cross validation. We have 190 countries with data so we will split our data into 19 folds to have 10 observations in each.
...
After performing 19-fold cross validation we obtain an average $R^2$ value of approximately `r round(mean(r_sq_results), 3)`. This is very similar to our training $R^2$ value of 0.6 indicating our model is neither overfitting or underfitting.
