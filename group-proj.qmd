...
We had a total of 33 NA's left, all from the Health Spending data, this table shows a few of those
```{r}
joined_data_long |>
  mutate(has_NAs = is.na(HealthSpendingPerCapita)) |>
  filter(has_NAs) |>
  slice_sample(n = 5) |>
  kable()
```
