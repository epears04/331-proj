...
=======
We had a total of 33 missing values from health spending per person.The main countries that had missing values for health care spending per capita were Afghanistan from 1995-2001 as well as Zimbabwe and Somalia from 2002-2010. We believe that these numbers are missing due to ongoing conflict in these countries during those time periods. 
There were a couple of other countries with missing values for health care spending per capita, however, they appear to be missing at random since they are one-offs.
...
joined_data_long |>
  ...
  labs(x = "Average Health Spending Per Person, Adjusted (US Dollars)",
       subtitle = "Average Life Expectancy (Years)",
       title = "Relationship Between Life Expectancy and Health Care Spending per Capita") +
...
joined_data_long |>
  ...
  ggtitle('Life Expectancy vs Natural Logarithm of Health Spending by Country',
          subtitle = 'Now Showing {closest_state}') +
  labs(x = "Natural Logarithm of Health Spending Per Capita",
       y = "Life Expenctancy at Birth")
...
data_transform <- 
  ...
  summarize(avg_life_exp = mean(LifeExpAtBirth),
            avg_health_spending = mean(HealthSpendingPerCapita)) |>
  mutate(adj_avg_life_exp = avg_life_exp,
         adj_avg_health_spending = log(avg_health_spending))

data_transform_lm <- lm(adj_avg_life_exp ~ adj_avg_health_spending, 
                        data = data_transform)
data_transform_lm |>
  tbl_regression(intercept = TRUE)
```
For every 10% increase in the average health spending per capita, the average life expectancy increases by about 0.33 years or about 4 months. 
